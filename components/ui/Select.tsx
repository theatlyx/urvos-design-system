...
export const SelectTrigger = forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger> & VariantProps<typeof selectTriggerVariants> & { error?: boolean }
>(({ className, children, state, error, ...props }, ref) => {
  const currentState = error ? "error" : state;
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={clsx(selectTriggerVariants({ state: currentState }), className)}
      {...props}
    >
      {children}
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Separator
    ref={ref}
    className={clsx("-mx-1 my-1 h-px bg-urvos-border", className)}
    {...props}
  />
));
